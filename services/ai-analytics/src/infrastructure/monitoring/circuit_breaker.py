...
class CircuitBreaker:
    ...
    async def is_blocked(self, company_id: Optional[str] = None, enabled: bool = True) -> bool:
        """Checks if the workflow should be halted."""
        state = await self.get_state(company_id)
        if state == CircuitState.KILLED:
            return True
        if state == CircuitState.OPEN and enabled:
            return True
        return False

    async def is_open(self, company_id: Optional[str] = None, enabled: bool = True) -> bool:
        """Backward compatibility for is_open()."""
        return await self.is_blocked(company_id, enabled=enabled)
