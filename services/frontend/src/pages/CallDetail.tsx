      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <section className="flex-1 flex flex-col relative bg-background-light dark:bg-background-dark overflow-hidden">
          <div className="shrink-0 bg-surface-light dark:bg-surface-dark border-b border-neutral-200 dark:border-neutral-700 p-6 z-10 shadow-sm">
            <div className="relative h-16 w-full mb-4 group cursor-pointer bg-neutral-100 dark:bg-neutral-800 rounded">
               {/* Waveform placeholder */}
               <div className="absolute inset-0 flex items-center justify-center text-slate-400 text-xs">Audio Waveform Visualizer</div>
               <div className="absolute top-0 bottom-0 left-0 bg-primary opacity-20" style={{ width: '17%' }}></div>
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <button className="text-neutral-500 hover:text-primary"><span className="material-icons">replay_10</span></button>
                <button className="h-10 w-10 bg-primary text-white rounded-full flex items-center justify-center shadow-lg"><span className="material-icons">play_arrow</span></button>
                <button className="text-neutral-500 hover:text-primary"><span className="material-icons">forward_10</span></button>
                <div className="text-sm font-mono font-medium text-neutral-600 dark:text-neutral-300 ml-2">
                   03:45 / {Math.floor(call.duration / 60)}:{(call.duration % 60).toString().padStart(2, '0')}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {transcript?.segments?.map((seg: any, i: number) => (
              <div key={i} className={`group flex gap-4 ${seg.start <= 225 ? 'opacity-100' : 'opacity-60'}`}>
                ...
                <div className="flex-1">
                  ...
                  <p className={`text-neutral-800 dark:text-neutral-200 leading-relaxed ${seg.start <= 225 && seg.end >= 225 ? 'bg-primary/10 p-2 rounded-lg border border-primary/20 font-medium' : ''}`}>
                    {seg.text}
                  </p>
                </div>
              </div>
