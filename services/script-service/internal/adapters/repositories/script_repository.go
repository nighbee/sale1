...
func (r *ScriptRepository) Create(ctx context.Context, id, companyID, name, filePath, parsedText, definition string) error {
	query := `
		INSERT INTO scripts_schema.scripts
		(id, company_id, name, file_path_minio, parsed_text, definition, version, is_active)
		VALUES ($1, $2, $3, $4, $5, $6, 1, true)
	`
	_, err := r.db.ExecContext(ctx, query, id, companyID, name, filePath, parsedText, definition)
	return err
}
