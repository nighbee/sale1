...
func (m *MockIntegrationRepository) GetByType(ctx context.Context, it domain.IntegrationType) (*domain.Integration, error) {
...
func (m *MockIntegrationRepository) List(ctx context.Context) ([]*domain.Integration, error) {
	args := m.Called(ctx)
	return args.Get(0).([]*domain.Integration), args.Error(1)
}
...
func (m *MockIntegrationRepository) Delete(ctx context.Context, it domain.IntegrationType) error {
	args := m.Called(ctx, it)
	return args.Error(0)
}
...
func TestTestConnection_InvalidType(t *testing.T) {
	...
	repo.On("GetByType", mock.Anything, domain.IntegrationType("invalid_type")).Return(nil, errors.New("not found"))

	err := uc.TestConnection(context.Background(), "invalid_type", nil, nil)
	assert.Error(t, err)
}
...
func TestTestConnection_NoCredentials(t *testing.T) {
	...
	repo.On("GetByType", mock.Anything, domain.IntegrationSipuni).Return(nil, errors.New("not found"))

	err := uc.TestConnection(context.Background(), domain.IntegrationSipuni, nil, nil)
	assert.Error(t, err)
...
func TestTestConnection_Sipuni_MissingKey(t *testing.T) {
	...
	err := uc.TestConnection(context.Background(), domain.IntegrationSipuni, creds, nil)
	assert.Error(t, err)
...
func TestTestConnection_AIProvider_MissingKey(t *testing.T) {
	...
	err := uc.TestConnection(context.Background(), domain.IntegrationOpenAI, creds, nil)
	assert.Error(t, err)
